...
describe("parseChanges", () => {
  it("groups versions, sections and bullets", () => {
    ...
    expect(entries.map((e) => e.version)).toEqual(["3.4.9", "3.4.8"]);
  ...
  it("parses the server updates/changes file", () => {
    ...
    expect(entries[0]?.version).toBe("3.4.9");
...
describe("unseenChangelog", () => {
  ...
  it("shows only the current version on first launch", () => {
    expect(unseenChangelog("3.4.9", entries).map((e) => e.version)).toEqual(["3.4.9"]);
  });
  ...
  it("shows versions newer than the last seen one", () => {
    ...
    expect(unseenChangelog("3.4.9", entries).map((e) => e.version)).toEqual(["3.4.9", "3.4.8"]);
